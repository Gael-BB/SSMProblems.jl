...
function main()
    ...
    open(OUTPUT_FILE, "w") do io
        # Write header
        write(io, "seed,algorithm,squared_error,effective_sample_size,runtime\n")
        
        @showprogress "Benchmarking..." for seed in 1:NUM_TESTS
            for algo in ALGORITHMS
                try
                    # Run experiment
                    # run_experiment returns: (sq_error, ess_val, elapsed_time)

                    sq_err, ess_val, time = TARGET_MODULE.run_experiment(seed, algo)
                    
                    # Write row
                    write(io, "$seed,$(string(algo)),$sq_err,$ess_val,$time\n")
                    flush(io)
                catch e
